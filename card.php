...
if (isset($_POST['submit'])) {
    ...
    $stmt = mysqli_prepare($conn, "SELECT * FROM card WHERE card_no = ?");
    mysqli_stmt_bind_param($stmt, "s", $cardno);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);

    if (mysqli_num_rows($result) > 0) {
        echo "<script>alert('Card already exists');window.location.href='card.php';</script>";
    }else {
        $sql = "INSERT INTO card (customer_id, card_no, card_holder, exp_date, cvv) VALUES ('$userid', '$cardno', '$cardname', '$exp', '$cvv')";
        $res = mysqli_query($conn, $sql);
        if ($res) {
            echo "<script>alert('Card registered successfully!');window.location.href='profile.php';</script>";
        }else {
            echo "<script>alert('Card registration failed.');window.location.href='profile.php';</script>";
        }
    }
}
?>
            ...
            <div class="back">
                <svg version="1.1" id="cardback" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                    ...
                    C707.1,246.4,704.4,249.1,701.1,249.1z" />
                        ...
                        <text transform="matrix(1 0 0 1 59.5073 228.6099)" id="svgnameback" class="st12 st13">NAME</text>
                    </g>
                </svg>
            </div>
        </div>
